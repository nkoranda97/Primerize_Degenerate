...
def _draw_assembly(sequence, primers, COL_SIZE):
    ...
    for i in range(N_primers):
        ...
        if seg_dir == 1:
            ...
            seq_line[seg_start : seg_end + 1] = map(
                util.reverse_complement, sequence[seg_start : seg_end + 1]
            )
    ...
    for i in range(int(math.floor((len(sequence) - 1) / COL_SIZE)) + 1):
        ...
        for j in range(len(seq_lines)):
            if (
                len(bp_lines[j][end_pos:].replace(" ", ""))
                and ("|" not in bp_lines[j][end_pos:].replace(" ", ""))
                and (not len(bp_lines[j][:start_pos].replace(" ", "")))
            ):
...
def _draw_common(fragments, labels):
    ...
    if len(fragments[0]) >= len(labels[0]):
        ...
        illustration_3 += "\033[91m%s%s\033[0m" % (
            labels[0],
            " " * (len(fragments[0]) - len(labels[0])),
        )
    elif fragments[0]:
        illustration_1 += (
            "\033[91m"
    ...
    if len(fragments[1]) >= len(labels[1]) + len(labels[2]):
        ...
        illustration_3 += "\033[92m%s%s%s\033[0m" % (
            labels[1],
            " " * (len(fragments[1]) - len(labels[1]) - len(labels[2])),
            labels[2],
        )
    elif fragments[1]:
        if len(fragments[1]) >= len(labels[1]):
            ...
            illustration_3 += "\033[92m%s%s\033[0m" % (
                labels[1],
                " " * (len(fragments[1]) - len(labels[1])),
            )
        else:
...
def _draw_region(sequence, params):
    ...
    if end - start <= 40:
        ...
        fragments.append(
            sequence[start : start + 20] + "......" + sequence[end - 19 : end + 1]
        )
    ...
    labels = [
        "%d" % (1 - offset),
        "%d" % params["which_muts"][0],
        "%d" % params["which_muts"][-1],
        "%d" % (len(sequence) - offset),
    ]
    (illustration_1, illustration_2, illustration_3) = _draw_common(fragments, labels)
    return {
        "labels": labels,
        "fragments": fragments,
        "lines": (illustration_1, illustration_2, illustration_3),
    }
...
def _draw_str_region(sequence, structures, bps, warnings, params):
    ...
    return {
        "labels": labels,
        "fragments": fragments,
        "lines": (illustration_3, illustration_2, illustration_1, illustration_str),
    }
