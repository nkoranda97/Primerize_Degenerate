...
class TestDesignSingle(unittest.TestCase):
    def setUp(self):
        self.job_1d = prm_1d.design(
            INPUT["SEQ_P4P6"],
            MIN_TM=INPUT["MIN_TM"],
            NUM_PRIMERS=INPUT["NUM_PRM"],
            MIN_LENGTH=INPUT["MIN_LEN"],
            MAX_LENGTH=INPUT["MAX_LEN"],
            prefix="primer",
        )
    ...
    def test_get(self):
        self.assertEqual(
            map(lambda x: list(x), self.job_1d.get("WARNING")), OUTPUT["1D"]["warning"]
        )
...
class TestDesignPlate2(unittest.TestCase):
    ...
    def test_get(self):
        self.assertEqual(
            self.job_2d.get("N_BP"), OUTPUT["2D"]["default"]["param"]["N_BP"]
        )
        self.assertEqual(
            self.job_2d.get("N_CONSTRUCT"),
            OUTPUT["2D"]["default"]["param"]["N_CONSTRUCT"],
        )
        self.assertEqual(
            self.job_2d.get("type"), OUTPUT["2D"]["default"]["param"]["type"]
        )


class TestDesignPlate3(unittest.TestCase):
    def setUp(self):
        self.job_3d = prm_3d.design(
            INPUT["SEQ_P4P6"],
            structures=[INPUT["STR_P4P6_1"], INPUT["STR_P4P6_2"]],
            primer_set=INPUT["PRIMER_SET_P4P6"],
            offset=INPUT["OFFSET_P4P6"],
            which_muts=which_muts,
            which_lib=[int(INPUT["LIB_P4P6"])],
            prefix="primer",
            is_single=True,
            is_fillWT=True,
            is_force=True,
    ...
    def test_get(self):
        self.assertListEqual(
            self.job_3d.get("STRUCTURE"), [INPUT["STR_P4P6_1"], INPUT["STR_P4P6_2"]]
        )
