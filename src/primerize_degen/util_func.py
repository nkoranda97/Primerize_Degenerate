    ...
    for key in sorted(plate._data):
        ...
        if not isinstance(mut, str):
            ...
            if mut[-2:] == "WT":
                ...
                string += (
                    "%s\033[96m%s\033[0m\033[93m%s\033[0m\033[91m%s\033[0m"
                    % (mut[:5], mut[5], mut[6:-1], mut[-1])
                ).ljust(50)
...
def _print_pair_mismatch_warning(sequence, warnings, offset):
    ...
    for pair in warnings:
        string += (
            "\033[93mWARNING\033[0m: Mismatch in base-pair between \033[96m%s\033[0m\033[93m%s\033[0m and \033[96m%s\033[0m\033[93m%s\033[0m.\n"
            % (
                sequence[pair[0] - 1],
                pair[0] - offset,
                sequence[pair[1] - 1],
        ...
        for p in range(len(plates)):
            ...
            if num_primers_on_plate:
                if num_primers_on_plate == 1 and "A01" in primer_sequences:
                    ...
                    if (not tag) or (isinstance(tag, str) and "WT" in tag):
                        continue

                file_name = os.path.join(
                    path,
                    "%s_plate_%d_primer_%d.svg"
                    % (primer_sequences.tag[:-1], k + 1, p + 1),
                )
...
def _save_structures(structures, warnings, sequence, offset, name, path="./"):
    ...
    if warnings:
        ...
        for pair in warnings:
            lines.append(
                "Mismatch in base-pair between %s%d and %s%d."
                % (
                    sequence[pair[0] - 1],
                    pair[0] - offset,
                    sequence[pair[1] - 1],
            ...
            if num_primers_on_plate:
                ...
                for i, row in enumerate(sorted(primer_sequences._data)):
                    ...
                    if not isinstance(tag, str):
                        format = (
                            "font: color blue,"
                            if (not tag or primer == ref_primer[p])
                        ...
                        format = (
                            "font: color blue," if "WT" in tag else "font: color black,"
                        )

                    sheet.write(
                        i + 1,
                        0,
                        util.num_to_coord(row),
                        xlwt.easyxf(format + "  italic 1"),
                    )
                    sheet.write(i + 1, 1, tag, xlwt.easyxf(format))
                    sheet.write(i + 1, 2, primer, xlwt.easyxf(format))

        if len(workbook._Workbook__worksheets):
            workbook.save(file_name)


def _mutate_primers(
    plates, primers, primer_set, offset, constructs, which_lib=1, is_fillWT=False
):
    for i, mut in enumerate(constructs):
        ...
        for p in range(len(primer_set)):
            ...
            mut_primer = (
                util.reverse_complement(wt_primer) if primers[2, p] == -1 else wt_primer
            )
            for k, seq in mut:
                k = k + offset - 1
                if k >= primers[0, p] and k <= primers[1, p]:
                    ...
                    if seq[0] != mut_primer[m_shift]:
                        raise ValueError(
                            '\033[41mERROR\033[0m: Mismatch of \033[94mMutation\033[0m %s with input sequence "\033[95m%s\033[0m" at positon \033[92m%d\033[0m.\n'
            ...
            mut_primer = (
                util.reverse_complement(mut_primer)
                if primers[2, p] == -1
                else mut_primer
            )
            if mut_primer != wt_primer or is_fillWT:
                well_name = (
                    "Lib%d-%s" % (which_lib, mut)
                    if mut_primer != wt_primer
                    else "Lib%d-%s" % (which_lib, "WT")
                )
                plates[p][plate_num].set(well_tag, mut, mut_primer)

        for m in is_valid:
            print(
                "\033[93mWARNING\033[0m: Unmatched (or out of bound) \033[94mMutation\033[0m position \033[92m%s\033[0m in construct %s.\n"
                % (m, mut)
            )
