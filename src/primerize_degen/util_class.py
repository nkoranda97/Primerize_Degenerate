...
class Assembly(object):
    ...
    def __init__(self, sequence, primers, name, COL_SIZE=142):
        ...
        self.name = name
        (self.bp_lines, self.seq_lines, self.print_lines, self.Tm_overlaps) = (
            util_server._draw_assembly(self.sequence, self.primers, COL_SIZE)
        )
    ...
    def __repr__(self):
        ...
        return (
            ...
            % (
                ...
                len(self.seq_lines),
                len(self.bp_lines),
                len(self.print_lines),
                repr(self.Tm_overlaps),
            )
        )
    ...
    def echo(self):
        ...
        for i, (flag, string) in enumerate(self.print_lines):
            if flag == "$" and "xx" in string:
                ...
                if flag == "^":
                    string = (
                        string.replace("A", "\033[94mA\033[0m")
                        .replace("G", "\033[94mG\033[0m")
                        .replace("C", "\033[94mC\033[0m")
                        .replace("T", "\033[94mT\033[0m")
                    )
                else:
                    string = (
                        string.replace("A", "\033[95mA\033[0m")
                        .replace("G", "\033[95mG\033[0m")
                        .replace("C", "\033[95mC\033[0m")
                        .replace("T", "\033[95mT\033[0m")
                    )
                output += string
            elif flag == "~":
    ...
    def save(self, path="./", name=None):
        ...
        for i, primer in enumerate(self.primer_set):
            ...
            lines += "%s\033[93m%s\033[0m\t%s\n" % (
                ...
                util_func._primer_suffix(i).replace(" R", primer).replace(" F", primer),
            )

        lines = (
            lines.replace("\033[0m", "")
            .replace("\033[100m", "")
            .replace("\033[92m", "")
            .replace("\033[93m", "")
            .replace("\033[94m", "")
...
class Plate_96Well(object):
    ...
    def __repr__(self):
        ...
        if len(self):
            return (
                "\033[94m%s\033[0m {\033[93m'coords'\033[0m: %s, \033[93m'data'\033[0m: \033[91mdict\033[0m(\033[91mtuple\033[0m * %d)}"
                % (self.__class__, " ".join(sorted(self.coords)), len(self._data))
            )
        else:
    ...
    def get(self, coord):
        ...
        if util.coord_to_num(coord) is None:
            raise AttributeError(
                "\033[41mERROR\033[0m: Illegal coordinate value \033[95m%s\033[0m for \033[94m%s.get()\033[0m.\n"
            ...
            raise KeyError(
                "\033[41mERROR\033[0m: Non-Existent coordinate value \033[95m%s\033[0m for \033[94m%s.get()\033[0m.\n"
                % (coord, self.__class__)
            )
    ...
    def set(self, coord, tag, primer):
        ...
        if util.coord_to_num(coord) is None:
            raise AttributeError(
                "\033[41mERROR\033[0m: Illegal coordinate value \033[95m%s\033[0m for \033[94m%s.set()\033[0m.\n"
                % (coord, self.__class__)
            )
        else:
    ...
    def save(self, ref_primer="", file_name="./plate.svg", title=""):
        ...
        pyplot.xticks(
            [x * 1.125 + 0.75 for x in range(12)],
            [str(x + 1) for x in range(12)],
            fontsize=14,
        )
        pyplot.yticks(
            [y * 1.125 + 0.75 for y in range(8)], list("ABCDEFGH"), fontsize=14
        )
        ...
        for i in range(8):
            for j in range(12):
                ...
                if util.num_to_coord(num) in self.coords:
                    ...
                    if (isinstance(tag, Mutation) and not tag) or (
                        isinstance(tag, str) and "WT" in tag
                    ):
...
class Mutation(object):
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, str) and other == "WT":
            return len(self) == 0
    ...
    def push(self, mut_str):
        ...
        for mut in mut_str:
            ...
            if seq_org == seq_mut:
                raise ValueError(
                    "\033[41mERROR\033[0m: Unchanged sequence identity by \033[94mMutation\033[0m \033[92m%s\033[0m.\n"
                    % mut
                )
    ...
    def merge(self, other):
        ...
        if not isinstance(other, Mutation):
            raise TypeError(
                "\033[41mERROR\033[0m: Illegal input type for \033[94m%s.merge()\033[0m.\n"
                % self.__class__
            )
    ...
    def list(self):
        ...
        return map(
            lambda x: "%s%s%s" % (self._data[x][0], x, self._data[x][1]),
    ...
    def echo(self):
        ...
        for mut in self.list():
            if mut[-2:] == "WT":
                ...
                output.append(
                    "\033[96m%s\033[0m\033[93m%s\033[0m\033[91m%s\033[0m"
                    % (mut[0], mut[1:-1], mut[-1])
                )
...
class Construct_List(object):
    ...
    def __repr__(self):
        ...
        if len(self):
            return "\033[94m%s\033[0m {\033[91mlist\033[0m(%s * %d)}" % (
                self.__class__,
                repr(Mutation()),
                len(self),
            )
    ...
    def __contains__(self, mut_list):
        ...
        if not isinstance(mut_list, Mutation):
            mut_list = Mutation(mut_list)
