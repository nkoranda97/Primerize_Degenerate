    ...
    def __init__(self, init_dict):
        for key in init_dict:
            if key not in [
                ...
                raise ValueError(
                    "\033[41mERROR\033[0m: Unrecognized key \033[92m%s\033[0m for \033[94m%s\033[0m.\n"
                    % (key, self.__class__)
                )
    ...
    def __repr__(self):
        ...
        return (
            ...
            % (
                ...
                self.name,
                self.is_success,
                repr(self.primer_set),
                repr(self._params),
                repr(self._data["misprime_score"]),
                repr(self._data["assembly"]),
                repr(self._data["warnings"]),
            )
        )
    ...
    def save(self, path="./", name=None):
        ...
        if self.is_success:
            ...
            raise UnboundLocalError(
                "\033[41mFAIL\033[0m: Result unavailable for \033[94m%s\033[0m where \033[94mis_success\033[0m = \033[41mFalse\033[0m.\n"
                % self.__class__
            )
    ...
    def echo(self, key=""):
        ...
        if self.is_success:
            ...
            if key == "misprime":
                output = ""
                for i in range(
                    int(math.floor(self._params["N_BP"] / self._params["COL_SIZE"])) + 1
                ):
                    output += "%s\n\033[92m%s\033[0m\n%s\n\n" % (
                        self._data["misprime_score"][0][
                            i * self._params["COL_SIZE"] : (i + 1)
                            * self._params["COL_SIZE"]
                        ],
                        self.sequence[
                            i * self._params["COL_SIZE"] : (i + 1)
                            * self._params["COL_SIZE"]
                        ],
                        self._data["misprime_score"][1][
                            i * self._params["COL_SIZE"] : (i + 1)
                            * self._params["COL_SIZE"]
                        ],
                    )
                ...
                for warning in self._data["warnings"]:
                    p_1 = "\033[100m%d\033[0m%s" % (
                        warning[0],
                        util_func._primer_suffix(warning[0] - 1),
                    )
                    p_2 = ", ".join(
                        "\033[100m%d\033[0m%s" % (x, util_func._primer_suffix(x - 1))
                        for x in warning[3]
                    )
                    output += (
                        "\033[93mWARNING\033[0m: Primer %s can misprime with %d-residue overlap to position %s, which is covered by primers: %s\n"
                ...
                for i, primer in enumerate(self.primer_set):
                    name = "%s-\033[100m%s\033[0m%s" % (
                        self.name,
                        i + 1,
                        util_func._primer_suffix(i),
                    )
                    output += "%s\033[93m%s\033[0m\t%s\n" % (
                        name.ljust(39),
                        str(len(primer)).ljust(10),
                        util_func._primer_suffix(i)
                        .replace(" R", primer)
                        .replace(" F", primer),
                    )
                ...
                raise AttributeError(
                    "\033[41mERROR\033[0m: Unrecognized key \033[92m%s\033[0m for \033[94m%s.echo()\033[0m.\n"
                    % (key, self.__class__)
                )
        else:
            raise UnboundLocalError(
                "\033[41mFAIL\033[0m: Result of key \033[92m%s\033[0m unavailable for \033[94m%s\033[0m where \033[94mis_cucess\033[0m = \033[41mFalse\033[0m.\n"
                % (key, self.__class__)
            )
...
class Design_Plate(object):
    ...
    def __init__(self, init_dict):
        ...
        if self.get("TYPE") == "Mutate-and-Map":
            self._data["illustration"] = util_server._draw_region(
                self.sequence, self._params
            )
        elif self.get("TYPE") == "Mutation/Rescue":
            self._data["illustration"] = util_server._draw_str_region(
                self.sequence,
                self.structures,
                self._data["bps"],
                self._data["warnings"],
            ...
            print(
                util_func._print_pair_mismatch_warning(
                    self.sequence, self._data["warnings"], self._params["offset"]
                )
            )
        else:
    ...
    def __repr__(self):
        ...
        structures = (
            "\033[95mstructures\033[0m = %s, \n" % repr(self.structures)
        ...
        return (
            ...
            % (
                ...
                structures,
                repr(self._params),
                repr(self._data["constructs"]),
                repr(self._data["assembly"]),
                repr(self._data["plates"]),
            )
        )
    ...
    def save(self, key="", path="./", name=None):
        ...
        if self.is_success:
            ...
            key = key.lower()
            if key == "table":
                util_func._save_plates_excel(
                    self._data["plates"], self.primer_set, name, path
                )
            elif key == "image":
                util_func._save_plate_layout(
                    self._data["plates"], self.primer_set, name, path
                )
            elif key == "constructs":
                util_func._save_construct_key(
                    self._data["constructs"], name, path, self._params["which_lib"]
                )
            elif key == "assembly":
                self._data["assembly"].save(path, name)
            elif key == "structures" and self.get("TYPE") == "Mutation/Rescue":
                util_func._save_structures(
                    self.structures,
                    self._data["warnings"],
                    self.sequence,
                    self._params["offset"],
                ...
                raise AttributeError(
                    "\033[41mERROR\033[0m: Unrecognized key \033[92m%s\033[0m for \033[94m%s.save()\033[0m.\n"
                    % (key, self.__class__)
                )
        else:
            raise UnboundLocalError(
            ...
            if key == "plate":
                ...
                for i in range(len(self._data["plates"][0])):
                    for j in range(len(self._data["plates"])):
                        output += (
                            "Plate \033[95m%d\033[0m; Primer \033[92m%d\033[0m\n"
                            % (i + 1, j + 1)
                        )
                ...
                if self.get("TYPE") == "Mutation/Rescue":
                    warnings = util_func._print_pair_mismatch_warning(
                        self.sequence, self._data["warnings"], self._params["offset"]
                    )
                    return (
                        structures
