...
class Primerize_2D(Singleton):
    ...
    def __init__(
        self, offset=0, which_muts=[], which_lib=1, COL_SIZE=142, prefix="lib"
    ):
        self.prefix = prefix
        self.offset = offset
    ...
    def design(
        ...
        plates = [
            [util_class.Plate_96Well(which_lib) for i in range(N_plates)]
                ...
                if m != -1:
                    constructs.push(
                        ...
                        % (
                            sequence[m],
                            which_muts[m_pos],
                            util.get_mutation(sequence[m], which_lib),
                        )
                    )

            plates = util_func._mutate_primers(
                plates, primers, primer_set, offset, constructs, which_lib
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="\033[92mPrimerize 2D Mutate-and-Map Plate Design\033[0m",
    ...
    group1.add_argument(
        ...
        type=int,
        choices=(1, 2, 3),
        help="Mutation Library Choices {1, 2, 3}",
        dest="which_lib",
        default=1,
    )
